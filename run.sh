...
# Directories
TERRAFORM_DIR="/Users/shravanchandraparikipandla/Documents/repo/adcash-test/exercise/terraform"
KEY_FILE_PEM="/Users/shravanchandraparikipandla/Documents/repo/adcash-test/exercise/my-aws-key.pem"
...
while true; do
  ...
  case $choice in
    1)
      ...
      # Extract the build server's IP from Terraform output
      BUILD_SERVER_IP=$(terraform output -raw build_server_public_ip)

      # SSH into the build server and run Ansible
      ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i $KEY_FILE_PEM ec2-user@"$BUILD_SERVER_IP" << EOF
        set -e
